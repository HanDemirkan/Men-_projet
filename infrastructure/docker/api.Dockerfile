...
# --- dev: full source + hot reload via `nest start --watch` ---
# Runs through turbo (not a plain `pnpm --filter dev`) so that workspace
# dependencies the API imports at runtime (@qr-platform/database,
# @qr-platform/validation) are built first, per turbo.json's `dependsOn: ["^build"]`.
FROM deps AS dev
COPY . .
ENV NODE_ENV=development
EXPOSE 4000
CMD ["pnpm", "exec", "turbo", "run", "dev", "--filter=@qr-platform/api"]

# --- build: compile the API and every workspace package it depends on ---
FROM deps AS build
COPY . .
RUN pnpm turbo run build --filter=@qr-platform/api...
